...
case 'add':
			...
			$sql ="insert into account_details (date,apply,revenue,expense) values ('".$date."','".$apply."','".$revenue."','".$expense."')";
		$res =mysql_query($sql);
	if($res){
		$id = mysql_insert_id();
...
case 'edit':
			...
			$revenue = readPOST('revenue');
			$expense = readPOST('expense');
			$sql ="update account_details set date='$date', apply='$apply', revenue='$revenue' ,expense='$expense' where ID=$id";
	if(mysql_query($sql)){
